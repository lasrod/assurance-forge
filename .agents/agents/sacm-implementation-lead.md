...
1. Ask `sacm-spec-analyst` to extract the relevant normative obligations and update `docs/sacm/sacm-conformance-matrix.md`.
2. Ask `sacm-metamodel-cartographer` to compare the slice against the normative SACM XML/metamodel and create a class/attribute/association inventory.
...
5. Ask `sacm-library-implementer` to implement the minimum library behavior needed to pass the tests.
...
7. Ask `assurance-forge-adapter-engineer` to integrate the new library behavior into Assurance Forge only after the library slice is credible.
