...
| GSN3-MOD-003 | Part 1, Modular Extension | Away Goal | partial | supported | absent | absent | partial | Import can map the type to a cited Claim. Authoring cites a goal in another module from the Add menu, recording SACM `isCitation` and `citedElement`, and offering only goals that can legally be cited: `src/core/element_factory.cpp`, `src/core/commands/element_commands.cpp`, `src/ui/element_context_menu.cpp`, `tests/test_away_goal.cpp`. A citation inside one module is refused rather than drawn as away, because it would report support from a module that does not carry it. Neither renderer draws the away notation yet, and an away goal cannot be staged into a draft. |
