...
| GSN3-CORE-009 | Part 1, Core GSN | Undeveloped decorator | supported | supported | supported | partial | partial | `src/ui/panels/element_panel.cpp`; `src/ui/gsn/gsn_shapes.cpp`; `tests/test_gsn_svg_exporter.cpp`. Validation warns when the decorator survives on an element that has since been supported (`tests/test_gsn_wellformedness.cpp`); a challenge is correctly not counted as support. Whether an *undecorated* unsupported leaf should be flagged is left to the SCCG catalog. |
