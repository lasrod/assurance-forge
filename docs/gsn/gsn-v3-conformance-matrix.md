...
| GSN3-CORE-002 | Part 1, Core GSN | Strategy element and its inference role | supported | supported | supported | partial | partial | `src/core/element_factory.cpp`; represented by SACM `ArgumentReasoning` plus one inference. Validation rejects a Strategy wired as an end of an inference rather than as its reasoning (`tests/test_gsn_wellformedness.cpp`). That is a GSN rule and not a SACM one: `AssertedRelationship`'s source and target are typed `ArgumentAsset`, which `ArgumentReasoning` is, so SACM holds the wrong shape without objecting — which is why it needs checking here. "A Strategy must be supported" is not checked. |
| GSN3-CORE-003 | Part 1, Core GSN | Solution as a reference to evidence | supported | supported | supported | partial | partial | `src/core/element_factory.cpp`; `tests/test_assurance_tree.cpp`; represented by `ArtifactReference` plus `AssertedEvidence`. Validation rejects a non-Solution standing in the evidence position (`tests/test_gsn_wellformedness.cpp`); whether the artefact actually discharges the goal is a human judgement. |
...
| GSN3-CORE-007 | Part 1, Core GSN | SupportedBy relationship and direction | supported | supported | supported | partial | partial | `src/core/tree_editing.cpp`; `tests/test_tree_editing.cpp`; endpoints are reversed when translating between GSN and SACM direction. Validation reports an endpoint naming an element that is not in the case, resolving by `id`, `gid` and `#id` (`tests/test_gsn_wellformedness.cpp`); such a relationship is otherwise silently absent from the drawn argument. |
| GSN3-CORE-008 | Part 1, Core GSN | InContextOf relationship and direction | supported | supported | supported | partial | partial | `src/core/tree_editing.cpp`; `tests/test_tree_editing.cpp`; endpoints are reversed when translating between GSN and SACM direction. Validation rejects a context declared on anything but a Goal or a Strategy (`tests/test_gsn_wellformedness.cpp`). |
| GSN3-CORE-009 | Part 1, Core GSN | Undeveloped decorator | supported | supported | supported | partial | partial | `src/ui/panels/element_panel.cpp`; `src/ui/gsn/gsn_shapes.cpp`; `tests/test_gsn_svg_exporter.cpp`. Validation warns when the decorator survives on an element that has since been supported (`tests/test_gsn_wellformedness.cpp`); a challenge is correctly not counted as support. Whether an *undecorated* unsupported leaf should be flagged is left to the SCCG catalog. Authoring is offered on Goals and Strategies only: SACM records the decorator in the same single `assertionDeclaration` that carries `assumed`/`axiomatic`, and an Assumption or Justification is reached by `InContextOf`, never `SupportedBy`, so it has no missing support to declare. Previously the control appeared on every claim and the edit was silently discarded on reload -- see `docs/sacm/sacm-gsn-mapping.md`. |
| GSN3-CORE-010 | Part 1, Core GSN | Every GSN element has exactly one notation identifier | partial | supported | supported | partial | partial | Node notation identifiers are independently imported, edited, rendered and SVG-exported through `assuranceForge.gsn.identifier` without changing SACM storage ids or references; empty and duplicate edits are rejected. `tests/test_element_factory.cpp`; `tests/test_element_edit_controller.cpp`; `tests/test_event_replayer.cpp`; `tests/test_gsn_svg_exporter.cpp`. Uniqueness is now also checked across an imported case rather than only at the point of edit (`tests/test_gsn_wellformedness.cpp`). Mandatory *presence* is still met by falling back to the SACM storage id rather than by requiring an authored identifier, and standard interchange remains open. |
...
| GSN3-CORE-012 | Part 1, Core GSN | Off-diagram element/reference notation | n/a | n/a | absent | n/a | n/a | Decided in #454: off-diagram is an SVG export concern. It marks where an export split across several SVG files continues, and the diagram it names is that other file (evidence and decision: `docs/sacm/sacm-gsn-mapping.md`, `docs/sacm/sacm-decisions-and-questions.md`). Nothing is stored in SACM, so model, authoring, validation and interchange do not apply, and the canvas shows the whole argument without splits. Splitting an export is not built yet. An off-diagram target is not an `ArgumentPackage`: a package is a GSN Module, reached through an Away Goal (GSN3-MOD-003). |
...
| GSN3-CORE-014 | Part 1, Core GSN | Structured argument is a directed acyclic support graph | partial | supported | supported | partial | partial | Edits reject cycles and imported cycles are reported against this requirement id; `src/core/problems/argument_cycles.cpp`; `tests/test_argument_cycles.cpp`. There is no complete GSN well-formedness validator. |
| GSN3-CORE-015 | Part 1, Core GSN | Only permitted element/relationship combinations are authored | supported | partial | supported | supported | partial | `src/core/problems/gsn_wellformedness.cpp`; `tests/test_gsn_wellformedness.cpp`; `tests/test_tree_editing.cpp`. Imported structures are checked against the Core connection rules — a Solution, Context, Assumption or Justification is a leaf, and only a Goal or a Strategy is supported or declared in context. Authoring enforces the same rule through one shared predicate used by the Add menu, the id planner and the proposal preview, which previously disagreed. Dropping a Solution onto a Strategy is now accepted, matching what the Add menu already created (`tests/test_tree_editing.cpp`). Create/edit stays `partial` for one remaining combination: a Strategy under a Strategy, which GSN permits and SACM would hold — `AssertedRelationship`'s source and target are typed `ArgumentAsset`, which `ArgumentReasoning` is — but the single-inference encoding records the goal a strategy supports in a `strategyTarget` tag and nothing has established what that means when the target is itself a strategy. A limit of the encoding, not of the notation. |
...
| GSN3-MOD-003 | Part 1, Modular Extension | Away Goal | partial | supported | supported | partial | partial | Import can map the type to a cited Claim. Authoring cites a goal in another module from the Add menu, recording SACM `isCitation` and `citedElement`, and offering only goals that can legally be cited: `src/core/element_factory.cpp`, `src/core/commands/element_commands.cpp`, `src/ui/element_context_menu.cpp`. Both renderers draw the module in a compartment across the bottom of the shape and resolve the goal's statement through the citation rather than copying it: `src/ui/gsn/gsn_shapes.cpp`, `src/export/svg_writer.cpp`, `tests/test_away_goal.cpp`. A citation inside one module is refused rather than drawn as away, because it would report support from a module that does not carry it. Validation reports a citation that resolves to nothing and an away goal that is also argued in this module, each naming GSN3-MOD-003 (`src/core/problems/gsn_wellformedness.cpp`); neither offers a quick fix, because clearing a citation and deleting someone's argument are both decisions for a person. Developing an away goal inside the citing module is an error, because GSN forbids it (v2 Annex B1.3.1.1), and a module name two packages share falls back to the package id, because a module identifier must be unique (v2 2.5.2.6). The cross-module consistency and substitution rules of GSN3-MOD-014 are not checked, which is why this is partial. An away goal cites only goals in the same SACM file, by decision in #454: a reference into another file is an XMI external reference, which the library reports and drops on load (SACM23-XMI-003). An away goal cannot be staged into a draft. |
...
| GSN3-MOD-006 | Part 1, Modular Extension | Away Assumption | partial | supported | supported | partial | partial | A Claim declared `assumed` carrying `isCitation` and `citedElement`, classified by its declaration so it is not mistaken for an away goal. Authoring cites an assumption in another module from the Add menu, in context of the selected goal or strategy, offering only assumptions: `src/core/element_factory.cpp`, `src/core/commands/element_commands.cpp`, `src/ui/element_context_menu.cpp`. The audit event records the kind, and replay reproduces it (`tests/test_event_replayer.cpp`). Both renderers draw the Assumption shape with its module and read the statement through the citation (`tests/test_away_assumption_justification.cpp`). Validation reports a citation that resolves to nothing under this requirement (`src/core/problems/gsn_wellformedness.cpp`). Partial for the same reasons as GSN3-MOD-003: GSN3-MOD-014's cross-module rules are not checked, citations stay within one SACM file, and it cannot be staged into a draft. |
| GSN3-MOD-007 | Part 1, Modular Extension | Away Justification | partial | supported | supported | partial | partial | As GSN3-MOD-006, for a Claim carrying the justification role: authored from the Add menu offering only justifications in other modules, drawn with its module by both renderers, and an unresolved citation reported under this requirement (`tests/test_away_assumption_justification.cpp`). Partial for the same reasons. |
...
| GSN3-DIA-003 | Part 1, Dialectic Extension | Challenge targets a GSN element | supported | supported | supported | partial | partial | `tests/test_dialectic_challenge.cpp`; dashed open-arrow notation and side layout are tested. Validation reports a challenge whose target is not in the case — it is stored but never drawn against anything (`tests/test_gsn_wellformedness.cpp`). The well-formedness checker deliberately exempts counter relationships from the support rules, since a challenge is not support and GSN v3 lets one point at a relationship. |
...
Independent GSN notation identifiers and the first tranche of a
requirement-traceable well-formedness validator are now in place. The validator
...
1. Core off-diagram notation (GSN3-CORE-012), decided in #454 to be an SVG
   export concern: splitting an export across several SVG files and marking
   where each continues. Nothing about it is stored in SACM;
2. statement-form validation (GSN3-CORE-011), which needs a decision on what a
   tool may assert about a proposition before any code; and
3. extending the validator to the Pattern, Modular and Confidence extension
   rules, each of which first needs the carrier its matrix row above describes.
