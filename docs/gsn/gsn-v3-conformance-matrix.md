...
| GSN3-CORE-012 | Part 1, Core GSN | Off-diagram element/reference notation | blocked | blocked | blocked | blocked | blocked | Blocked rather than merely absent: the decorator's payload is a reference to another *diagram*, and no diagram construct exists in GSN Metamodel v2.2, in SACM 2.3, or in this tool. The canvas partitions by `ArgumentPackage`, which is GSN's Module — a different concept — so equating the two would answer a Core requirement with the Modular extension's semantics and pre-empt the Away Goal design. Evidence and the open decision: `docs/sacm/sacm-gsn-mapping.md` and `docs/sacm/sacm-decisions-and-questions.md`; the standards defect is gap row 14 in `docs/sacm/sacm-gsn-metamodel-gaps.md`. This is a Core GSN feature, not a pattern feature or merely a report-layout convenience. |
...
1. Core off-diagram notation (GSN3-CORE-012) is **blocked, not queued**. It was
   the one Core construct with no model, renderer or export at all; investigating
   it showed why. The decorator references another *diagram*, and no diagram
   exists to reference — not in GSN Metamodel v2.2, not in SACM 2.3, and not in
   this tool. Defining one is an open project decision, recorded in
