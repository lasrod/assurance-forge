...
| AF-AI-009 | Propose changes to a safety case from an external AI client | supported | src/core/changesets/change_set.cpp, src/core/changesets/change_set_store.cpp, src/agent/change_operations.cpp, src/app/app_runtime_project.cpp | tests/test_change_sets.cpp, tests/test_change_set_acceptance.cpp | An agent builds a **change set** the user watches take shape: from its first staged operation the GSN canvas draws the argument as it would be if accepted, with additions, edits and removals marked in place by border and badge (never colour alone). Staging changes nothing — no SACM write, no command, no audit transaction — and a change set that would not apply is refused at staging time, so the canvas can always draw one. **Acceptance is an ordinary audited edit**: it goes through the unchanged `ApplyProposalCommand`, so it is undoable, replayable and attributed to the connecting client. No agent-specific command type exists, which is what guarantees an agent cannot make a change the application could not make itself. Staleness is re-checked at acceptance, not at staging, because the user may edit the argument while reading the proposal. Accept and Reject live in the Review panel; there is no tool an agent can call to apply its own work. Replaces the earlier design in which the adapter wrote proposal files into the project, making it a second writer of `af.proj` and `review-items.af.json` that the application overwrote from its own older copy on the next save. |
| AF-AI-010 | SCCG guidance and checks for an external AI client | supported | src/mcp/guidance.cpp, src/core/sccg/staged_checks.cpp, src/core/guideline_catalog.cpp | tests/test_sccg_staged_checks.cpp, tests/test_mcp_server.cpp | Three mechanisms. **Resources**: `sccg://guidelines` publishes the catalog over MCP. **Prompts**: `draft_argument_from_standard`, `add_argumentation` and `restructure_case` each carry the guidance for that job, quoted from the catalog so prompt and guideline cannot drift — this is what makes an agent aware of the rules before it writes. **Checks**: findings returned in the result of every staging call and shown on the change set. The mechanical set is deliberately narrow and individually tested — EV.1 (claim with no support and no undeveloped marker), AR.2 (strategy developing into nothing), AR.1 (solution with children; support cycles), CL.5 (unbounded qualifier from the list SCCG names). Most of SCCG is prose only a reader can judge, so this is **not** "SCCG compliance": findings are advisory and never block acceptance. |
