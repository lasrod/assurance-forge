...
| AF-AI-007 | MCP server — build safety cases from an external AI client | prototype | src/mcp/server.cpp, src/mcp/tools.cpp, src/mcp/session.cpp | tests/test_mcp_server.cpp, tests/test_mcp_proposals.cpp, tests/test_mcp_jsonrpc.cpp | Design: docs/features/mcp-server.md. `assurance-forge-mcp` speaks JSON-RPC 2.0 over stdio. An external client can read a case (overview, search, element detail, GSN tree) and propose changes to it, which land as review proposals the user accepts in the app — the agent never edits the case, so it cannot make a change nobody looked at, nor one the app could not make itself. `prototype` rather than `supported` because enabling it still means hand-editing `mcp.enabled` into the settings file. Sharing is gated by that flag, which fails closed and is verified through the real process by cmake/run_mcp_smoke_test.cmake. Independent of AF-AI-001..006: the layer gate forbids `mcp/` from including `ai/`, so the two features cannot share an inference path. |
