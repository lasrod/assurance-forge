...
| AF-AI-007 | MCP server — build safety cases from an external AI client | in-development | src/mcp/server.cpp, src/mcp/tools.cpp | tests/test_mcp_server.cpp, tests/test_mcp_jsonrpc.cpp | Design: docs/features/mcp-server.md. Reading works end to end — `assurance-forge-mcp` speaks JSON-RPC 2.0 over stdio and serves case overview, element search, element detail and the GSN argument tree, so an external client can read and reason about a case. Building it cannot: the write surface (proposal drafts a human accepts) is not implemented, which is why this is not `prototype`. Sharing is gated by an `mcp.enabled` setting that fails closed, verified through the real process by cmake/run_mcp_smoke_test.cmake. Independent of AF-AI-001..006: the layer gate forbids `mcp/` from including `ai/`, so the two features cannot share an inference path. |
