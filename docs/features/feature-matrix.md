...
| AF-AI-009 | Propose changes to a safety case from an external AI client | planned | | | Superseded the earlier design in which the adapter wrote review-proposal files into the project directly. That made it a second writer of `af.proj` and `review-items.af.json`, which the application overwrote from its own older copy on the next save, so an agent's proposal silently vanished. Replaced by change sets built over the bridge and applied by the existing `ApplyProposalCommand`, so an agent's edit is audited, undoable and attributable exactly like one made with the mouse. Not available until that lands. |
