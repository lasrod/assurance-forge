...
| AF-AI-007 | MCP server — build safety cases from an external AI client | supported | src/mcp/server.cpp, src/mcp/tools.cpp, src/app/mcp_client_config.cpp | tests/test_mcp_server.cpp, tests/test_mcp_proposals.cpp, tests/test_mcp_client_config.cpp | Design: docs/features/mcp-server.md. `assurance-forge-mcp` speaks JSON-RPC 2.0 over stdio. An external client can read a case (overview, search, element detail, GSN tree) and propose changes to it, which land as review proposals the user accepts in the app — the agent never edits the case, so it cannot make a change nobody looked at, nor one the app could not make itself. Consent: ADR 0007. Preferences carries the consent toggle and a copyable client configuration, so enabling it no longer means hand-editing the settings file; that section was confirmed in a running app, including toggling consent on and seeing the flag reach disk without disturbing the sibling AI settings. Sharing is gated by a flag that fails closed and is verified through the real process by cmake/run_mcp_smoke_test.cmake. Independent of AF-AI-001..006: the layer gate forbids `mcp/` from including `ai/`, so the two features cannot share an inference path. |
