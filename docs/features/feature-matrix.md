...
| AF-AI-006 | SCCG-guided AI element review | supported | src/ai/ai_claim_review.cpp, src/app/controllers/ai_review_controller.cpp, src/app/app_runtime_ai.cpp | tests/test_ai_claim_review.cpp, tests/test_ai_review_controller.cpp | One `AI Review` action selects the unique SCCG 0.6 profile for the selected GSN role: claim, strategy, evidence, assumption, justification, context, or challenge. Selection fails closed if the catalog has no match or more than one match; contextual artifacts are not misclassified as evidence and counter elements use the challenge profile. |
...
| AF-AI-014 | One integrated working draft per argument file | planned | | | Replaces the two independent proposal mechanisms — an in-memory `ChangeSet` per MCP connection (AF-AI-009) and one persisted `ReviewProposal` file per review item (AF-METH-004) — with a single workspace per argument, holding ordered change groups from MCP, SCCG AI review, the user, and imported legacy proposals, materialized into one complete assurance case. The core domain, deterministic materialization, once-only identity allocation and `.af/drafts` recovery exist and are tested, but **no user-facing path reaches them yet**, which is why this is `planned` and cites nothing: the matrix records what a user can do, and today a user can do none of it. Raised only when the end-to-end acceptance scenario passes in the running application. See ADR 0009, ADR 0010 and docs/architecture/integrated-draft-workspace-plan.md. |
