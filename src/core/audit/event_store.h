...
public:
    ...
    const std::vector<AuditTransaction>& Transactions() const {
        return transactions_;
    }
    std::uint64_t LatestTransactionSequence() const {
        return latest_transaction_sequence_;
    }
    std::uint64_t LatestEventSequence() const {
        return latest_event_sequence_;
    }
    // sha256 hex of the entire on-disk log content (empty when the log is empty).
    const std::string& EventStoreHash() const {
        return event_store_hash_;
    }
    // sha256 hex of the most recently appended transaction line (empty for empty log).
    const std::string& LatestTransactionHash() const {
        return latest_transaction_hash_;
    }
    ...
    bool TornTailRecovered() const {
        return torn_tail_recovered_;
    }
    const std::string& TornTailDiagnostic() const {
        return torn_tail_diagnostic_;
    }
