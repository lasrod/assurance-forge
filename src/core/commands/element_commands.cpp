...
bool CreateAwayElementCommand::Apply(CommandContext& ctx, audit::AuditEvent& out_event, std::string& out_error) {
    const std::string what = std::string("the away ") + core::AwayElementKindName(kind_);
    ...
    // element behind with no citation -- which is an ordinary local element
    // stating what another module was meant to state.
    if (!core::CanAddAwayElement(ctx.model, &ctx.package, parent_id_, cited_id_, kind_, out_error))
        return false;

    std::string planned_element_id;
    std::string planned_relationship_id;
    if (!core::PlanAwayElementIds(ctx.model,
                                  &ctx.package,
                                  parent_id_,
                                  cited_id_,
                                  kind_,
    ...
    if (ctx.library_document != nullptr) {
        const sacm_adapter::AddChildOutcome outcome = sacm_adapter::apply_add_child(
            *ctx.library_document, parent_id_, LibraryChildKindFor(kind_), planned_element_id, planned_relationship_id);
        if (outcome.supported && !outcome.applied) {
            out_error = LibraryRejection(what, outcome.diagnostics);
            return false;
        }
        if (outcome.applied) {
            ...
            if (!citation.applied) {
                ...
                ctx.library_primary = true;
                out_error = LibraryRejection(what + " citation", citation.diagnostics);
                return false;
            }
            generated_id_ = outcome.new_element_id;
            generated_relationship_id_ = outcome.new_relationship_id;
    ...
    if (!applied_to_library) {
        const LibraryBridgeMutator mutate =
            [this](parser::AssuranceCase& model, sacm::AssuranceCasePackage& package, std::string& err) -> bool {
            return core::AddAwayElement(
                model, &package, parent_id_, cited_id_, kind_, generated_id_, generated_relationship_id_, err);
        };
        if (!ApplyLegacyOrRefuse(ctx, mutate, out_error))
            return false;
    }

    out_event.event_type = Name();
    out_event.payload = nlohmann::ordered_json::object();
    out_event.payload["parent_id"] = parent_id_;
    out_event.payload["cited_id"] = cited_id_;
    if (kind_ != AwayElementKind::Goal)
        out_event.payload["kind"] = core::AwayElementKindName(kind_);
    out_event.payload["generated_id"] = generated_id_;
    out_event.payload["generated_relationship_id"] = generated_relationship_id_;
    return true;
}
