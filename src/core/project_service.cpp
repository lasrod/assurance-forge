...
std::array<unsigned char, 32> Sha256Digest(const unsigned char* data, size_t size) {
    ...
    auto transform = [&state](const unsigned char* block) {
        ...
        for (size_t i = 16; i < 64; ++i) {
            schedule[i] = Sha256SmallSigma1(schedule[i - 2]) + schedule[i - 7] +
                          Sha256SmallSigma0(schedule[i - 15]) + schedule[i - 16];
        }
        ...
        for (size_t i = 0; i < 64; ++i) {
            uint32_t temp1 = h + Sha256Sigma1(e) + Choose(e, f, g) + kRoundConstants[i] + schedule[i];
            uint32_t temp2 = Sha256Sigma0(a) + Majority(a, b, c);
            h = g;
            g = f;
            f = e;
            e = d + temp1;
    ...
    for (size_t i = 0; i < remaining; ++i) {
        final_blocks[i] = data[offset + i];
    }
    final_blocks[remaining] = 0x80;

    size_t final_size = remaining + 1;
    if (final_size > 56) {
        final_size = 120;
    } else {
        final_size = 56;
    ...
    uint64_t bit_length = static_cast<uint64_t>(size) * 8U;
    for (size_t i = 0; i < 8; ++i) {
        final_blocks[final_size + i] = static_cast<unsigned char>((bit_length >> ((7 - i) * 8)) & 0xffU);
    }

    transform(final_blocks.data());
    if (final_size == 120) {
        transform(final_blocks.data() + 64);
    }
...
bool Sha256Bytes(const unsigned char* data, size_t size, std::string& hash, std::string& error) {
    (void)error;
    auto digest = Sha256Digest(data, size);
    hash = HexString(std::vector<unsigned char>(digest.begin(), digest.end()));
    return true;
}
