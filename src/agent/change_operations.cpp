...
// Operation JSON is parsed by `core::reviews`, so this surface and the SCCG
// review path cannot come to accept different things.
bool ParseOne(const nlohmann::json& source, core::reviews::PatchOperation& out, std::string& error) {
    return core::reviews::ParsePatchOperationJson(source, out, error);
}
