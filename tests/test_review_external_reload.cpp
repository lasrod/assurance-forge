...
std::filesystem::path ScratchFile(const std::string& stem) {
    const std::filesystem::path directory =
        std::filesystem::temp_directory_path() / "af_review_external_reload";
...
void WriteItemsExternally(const std::filesystem::path& path,
                          const std::vector<core::reviews::ReviewItem>& items) {
...
TEST(ReviewExternalReload, PicksUpAnItemAnotherProcessAppended) {
    ...
    WriteItemsExternally(
        path, {MakeItem("existing", "Already here"), MakeItem("mcp-proposal:p1", "From MCP")});
    WaitPastThrottle();

    EXPECT_TRUE(controller.ReloadIfChangedExternally())
        << "an externally appended review item was not noticed";
