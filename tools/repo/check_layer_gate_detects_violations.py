...
CASES = [
    ...
    ("parser must not include legacy_sacm", "parser", "reader.h", '#include "legacy_sacm/sacm_model.h"\n', True),
    # Added with the #341 rename. `sacm/` now names the reusable library only,
    # and parser must not reach that either. Two prefixes need two cases: one
    # case cannot fail for the other's rule, and a label that names the wrong
    # prefix makes the failure output point at the wrong gate.
    ("parser must not include the SACM library", "parser", "reader.h", '#include "sacm/model/document.h"\n', True),
