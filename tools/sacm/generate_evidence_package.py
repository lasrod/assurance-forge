...
Non-vacuity guards, in the spirit of the matrix gate this tool imports from:
  ...
  - The normative-source hashes are recomputed and compared against pins
    carried IN THIS TOOL (tracked and reviewed). The fetched SHA256SUMS is
    generated from whatever was downloaded, so comparing against it would
    verify nothing. The references themselves are fetched, not committed
    (OMG licensing note in scripts/fetch-sacm23-references.sh): gate mode
    notes their absence loudly and defers the comparison to the release
    workflow, which fetches them; package mode requires them outright.
...
# The expected SHA-256 of each normative OMG source. Pinned here -- in a
# tracked, reviewed file -- because third_party/ is gitignored and its
# SHA256SUMS is generated by the fetch script from whatever it downloaded:
# comparing a download against a hash derived from the same download would
# verify nothing. These are the values quoted in the compliance-points page
# and the completeness audit.
PINNED_SOURCES = {
    "SACM-2.3-formal-23-05-08.pdf": "8004a2ee2e3469552ae199deea22e44ec86a1d4769948bdce4782c7220de3012",
    "SACM-2.3-ptc-22-03-13.xml": "4bc12020fe38018fe24d39e56ec04f2fc9190fe7cf1161d85452fda5f3fbeb1e",
}
...
def verify_spec_pins(check_mode):
    """Recompute the fetched normative sources against the in-tool pins.

    A fresh checkout legitimately lacks the references (fetched, not
    committed). In check mode that is a loudly printed note, not a failure --
    the release workflow fetches them and runs the real comparison, so a wrong
    or absent reference cannot reach a release. Package mode requires them.
    """
    problems, notes, entries = [], [], []
    for name, expected in PINNED_SOURCES.items():
        path = SPEC_DIR / name
        entry = {"file": f"third_party/sacm-2.3/{name}", "sha256": expected}
        if not path.exists():
            if check_mode:
                notes.append(f"normative reference not fetched: {name} "
                             f"(scripts/fetch-sacm23-references.sh); pin verification runs in the release job")
                entry["verified"] = False
            else:
                problems.append(f"normative reference missing: {name}; run scripts/fetch-sacm23-references.sh")
                entry["verified"] = False
        else:
            actual = sha256_of(path)
            if actual != expected:
                problems.append(f"{name}: pinned {expected}, recomputed {actual}")
            entry["verified"] = actual == expected
...
def write_statement(out_dir, manifest):
    ...
    lines = [
        ...
        "  `manifest.json` are the pins carried in the generator itself, recomputed",
        "  against the OMG documents `scripts/fetch-sacm23-references.sh` fetches.",
...
def build_package(out_dir, args):
    ...
    if args.test_results:
        ...
        if not junit.exists():
            ...
            except (ET.ParseError, OSError, ValueError) as parse_error:
                # A crash here would bury the actionable message in a traceback;
                # a malformed results file is a problem like any other.
                problems.append(f"test results unreadable ({junit}): {parse_error}")
                test_totals = None
            if test_totals is not None:
...
def main():
    ...
    if args.check:
        ...
        with tempfile.TemporaryDirectory() as temp:
            problems, notes = build_package(Path(temp) / "evidence-package", args)
        for note in notes:
            print(f"  [note] {note}")
    ...
    problems, _ = build_package(out_dir, args)
    if problems:
        for problem in problems:
