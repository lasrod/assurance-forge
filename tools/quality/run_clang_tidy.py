...
def main() -> int:
    ...
    if args.check:
        ...
        # Checked before the comparison so the advice below can be trusted. A
        # baseline with no usable version cannot name one to install, and
        # printing `pip install clang-tidy==None` would send the reader after a
        # release that does not exist. "unknown" is what tool_version() records
        # when `clang-tidy --version` could not be parsed, so a baseline holding
        # it was written by a run that did not know its own tool either.
        baseline_version = baseline.get("tool_version")
        if not args.ignore_tool_version and (
            not isinstance(baseline_version, str) or baseline_version in ("", "unknown")
        ):
            fail(
                f"the baseline records no usable clang-tidy version (found {baseline_version!r}).\n"
                "It cannot be compared against, because there is no way to tell whether this\n"
                "run's clang-tidy is the one that wrote it. Regenerate it with a clang-tidy that\n"
                "reports its version:\n\n"
