...
std::optional<AssertionDeclaration> parse_assertion_declaration(std::string_view literal) {
    ...
    if (literal == "needsSupport")
        return AssertionDeclaration::NeedsSupport;
    if (literal == "assumed")
        return AssertionDeclaration::Assumed;
    if (literal == "axiomatic")
        return AssertionDeclaration::Axiomatic;
    if (literal == "defeated")
        return AssertionDeclaration::Defeated;
    if (literal == "asCited")
        return AssertionDeclaration::AsCited;
    return std::nullopt;
}
