...
std::vector<model::ElementId>
ids_with_change(const std::vector<ChangeRecord>& changes, ChangeRecord::Change first, ChangeRecord::Change second) {
...
std::vector<model::ElementId> MutationResult::deleted_ids() const {
    return ids_with_change(changes, ChangeRecord::Change::Deleted, ChangeRecord::Change::RelationshipDeleted);
}
